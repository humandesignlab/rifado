...
contract LottoCoin is usingOraclize {
  ...
  function buyTicket(uint _pickedNumber) public payable {
   ...
   function getTicketOwnersList(uint[] indexes) public view returns (address[], uint[], uint[]) {
     address[] memory ticketBuyers = new address[](indexes.length);
     uint[] memory pickedNumbers = new uint[](indexes.length);
     uint[] memory timeStamps = new uint[](indexes.length);

     for (uint i = 0; i < indexes.length; i++) {
       Ticket storage ticket = ticketOwnersList[indexes[i]];
       ticketBuyers[i] = ticket.ticketBuyer;
       pickedNumbers[i] = ticket.pickedNumber;
       timeStamps[i] = ticket.timeStamp;
     }

     return (ticketBuyers, pickedNumbers, timeStamps);
   }
